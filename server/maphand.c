                                 ...
                                 struct player *pplayer)
{
  ...
  for (i = 0; i < O_LAST; i++) {
    ...
    int x = city_tile_output(nullptr, ptile, FALSE, i);
                                      ...
                                      struct unit *funit)
{
  ...
  if (BORDERS_DISABLED != game.info.borders && pdcity == nullptr) {
    ...
    } else if (nullptr != owner && nullptr == pplayer) {
  ...
  if (pdcity != nullptr) {
    ...
    if (visible) {
      ...
      if (can_player_see_units_in_city(pplayer, pcity)) {
        ...
        if (pcity != nullptr && city_is_occupied(pcity)) {
...

  {
    ...
    dbv_to_bv(full.vec, &(plrtile->extras));
    infratext = get_infrastructure_text(full);
  ...
  if (visible) {
    ...
    if (punit != nullptr && pdcity == nullptr) {
      struct player *owner = unit_owner(punit);
      ...
      if (!pplayer || owner == pplayer) {
        ...
        struct vision_site *site = nullptr;

        if (hcity != nullptr) {
          site = map_get_player_city(city_tile(hcity), pplayer);
        }

        if (site != nullptr && site->identity == hcity->id) {
                              ...
                              int visible_unit_id, int focus_unit_id)
{
  ...
  struct unit *punit
    = (visible_unit_id == 0) ? nullptr : game_unit_by_number(visible_unit_id);
  struct unit *funit
    = (focus_unit_id == 0) ? nullptr : game_unit_by_number(focus_unit_id);
